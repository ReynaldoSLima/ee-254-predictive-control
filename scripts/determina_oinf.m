function [Sf, bf] = determina_oinf(Af, Gamma, Spsi, bpsi, max_iter,tol)
...
while((i <= max_iter) && (flag_redund == 0) )
    flag_redund = 1;
    % Se esse flag não for mudado, o algoritmo será encerrado
...
% Finalização
if (flag_redund == 0)
    % O algoritmo foi encerrado sem que houvesse
    % redundância das restrições
